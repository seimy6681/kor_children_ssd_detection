...
# )
class MultitaskWhisperForConditionalGeneration(WhisperGenerationMixin, WhisperPreTrainedModel):
    ...
    def forward(
        ...
        encoder_hidden = outputs.encoder_last_hidden_state # using the last encoder layer output # (B, T, D)
        encoder_output = encoder_hidden.mean(dim=1) # mean pooling across time steps (B, T, D)
        binary_logits = self.binary_classifier(encoder_output).squeeze(-1)

        attention_scores = self.second_attention_layer(encoder_output) # Shape: (B, T, 1)
        attended_output = attention_scores * encoder_output
        concat_output = torch.cat(
            [encoder_hidden, attended_output], dim=-1 # (B, T, 2D)
        )

        fused_output = self.fusion_layer(concat_output)

        outputs = self.model(
            input_features=None,
            encoder_outputs=BaseModelOutput(last_hidden_state=fused_output) # fused된 인코더 output 이용
            de
        )
        ...
        if binary_labels is not None:
            ...
            binary_loss = loss_fct(binary_logits, binary_labels)
