...
model = FusedMultitaskWhisperForConditionalGeneration.from_pretrained('openai/whisper-small')
...
for seed in range(42, 42 + 1): 
    ...
    name=f"global-fused-multitask-whisper-seed-{seed}",
    config={
        "learning_rate": 3e-4,
        "batch_size": 8,
    ...
    training_args = Seq2SeqTrainingArguments(
        seed=seed,
        output_dir=f"/data/selinawisco/whisper_finetuning_asr/global-fused-multitask-whisper-small-0.15-{seed}",  # change to a repo name of your choice
        save_total_limit=2,
