...
class DynamicAttendedBinaryMultitaskWav2Vec2ForCTC(Wav2Vec2ForCTC):
    """
    ...
    def __init__(self, config):
        ...
        self.alpha = getattr(config, "alpha", 0.15)
    ...
    def forward(
        ...
        if loss_asr is not None or loss_error is not None:
            loss = asr_weight.mean() * (loss_asr if loss_asr is not None else 0) + \
                self.alpha * ac_weight.mean() * (loss_error if loss_error is not None else 0)
