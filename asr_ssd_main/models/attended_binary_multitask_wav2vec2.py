...
class AttendedBinaryMultitaskWav2Vec2ForCTC(Wav2Vec2ForCTC):
    ...
    def forward(
        ...
        ).squeeze(1) # Shape: (batch_size, seq_len, hidden_size)
        ...
        # LOCAL ATTENTION
        # fused_states = attention_weights.unsqueeze(-1) * hidden_states
        # fused_states = torch.cat(
        #     [hidden_states, attended_states.expand(-1, hidden_states.size(1), -1)],
        #     dim=-1
        # ) 
