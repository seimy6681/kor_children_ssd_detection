...
class AgeAdversarialWav2Vec2ForCTC(Wav2Vec2ForCTC):
    ...
    def __init__(self, config, main_arg=None):
        ...
        if main_arg is not None:
            self.alpha = main_arg.multitask_alpha if main_arg.alpha is not None else 0.3
            self.reverse_alpha = main_arg.reverse_alpha if main_arg.reverse_alpha is not None else 0.5
        else:
            self.alpha = 0.0
            self.reverse_alpha = 0.0
    def forward(
            self,
        ...
        if reverse_alpha is None: # if scheduling is not used
            reverse_alpha = self.reverse_alpha
        
        pooled_hidden_states = hidden_states.mean(dim=1)
        domain_feature = ReverseLayerF.apply(pooled_hidden_states, reverse_alpha)
        # domain_feature = ReverseLayerF.apply(pooled_hidden_states, self.reverse_alpha)
        age_logits = self.age_classifier(domain_feature)
        ...
        if age_label is not None:            
            ...
            loss_age = loss_fn(domain_feature, age_label.to(age_logits.device))
